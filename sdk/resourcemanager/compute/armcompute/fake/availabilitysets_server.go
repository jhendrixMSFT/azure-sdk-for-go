...
type AvailabilitySetsServer struct {
	//onCreateOrUpdate func(ctx context.Context, resourceGroupName string, availabilitySetName string, parameters armcompute.AvailabilitySet, options *armcompute.AvailabilitySetsClientCreateOrUpdateOptions) (resp azfake.Responder[armcompute.AvailabilitySetsClientCreateOrUpdateResponse], err azfake.ErrorResponder)
	onCreateOrUpdate []Requester[availabilitySetsCreateOrUpdateParams, armcompute.AvailabilitySetsClientCreateOrUpdateResponse]
...
func (r *Responder[T]) Header(key, val string) *Responder[T] {
	return r
}

func (r *Responder[T]) Response(resp T) *Responder[T] {
	return r
}

func (r *Responder[T]) ResponseError(errorCode string, httpStatus int) *Responder[T] {
	return r
}

func (r *Responder[T]) Error(err error) *Responder[T] {
	return r
}

func (r *Responder[T]) Next() (T, error) {

}

type Requester[TArgs, TResp any] struct {
	args TArgs
	resp Responder[TResp]
}

func (r *Requester[TArgs, TResp]) Match(args TArgs) *Responder[TResp] {
	if reflect.DeepEqual(args, r.args) {
		return &r.resp
	}
	return nil
}
...
type availabilitySetsCreateOrUpdateParams struct {
	...
	Options             *armcompute.AvailabilitySetsClientCreateOrUpdateOptions
}

func (a availabilitySetsCreateOrUpdateParams) String() string {
...
func (a *AvailabilitySetsServer) OnCreateOrUpdate(ctx context.Context, resourceGroupName string, availabilitySetName string, parameters armcompute.AvailabilitySet, options *armcompute.AvailabilitySetsClientCreateOrUpdateOptions) (resp *Responder[armcompute.AvailabilitySetsClientCreateOrUpdateResponse]) {
	...
	/*a.onCreateOrUpdate = func(fnCtx context.Context, fnResourceGroupName string, fnAvailabilitySetName string, fnParameters armcompute.AvailabilitySet, fnOptions *armcompute.AvailabilitySetsClientCreateOrUpdateOptions) (azresp azfake.Responder[armcompute.AvailabilitySetsClientCreateOrUpdateResponse], err azfake.ErrorResponder) {
		if fnCtx != nil {
			if ctxErr := fnCtx.Err(); ctxErr != nil {
				err.SetError(ctxErr)
				return
			}
		}
		if !reflect.ValueOf(resourceGroupName).IsZero() && resourceGroupName != fnResourceGroupName {
			err.SetError(fmt.Errorf("resourceGroupName: want %s, got %s", resourceGroupName, fnResourceGroupName))
			return
		}
		if !reflect.ValueOf(availabilitySetName).IsZero() && availabilitySetName != fnAvailabilitySetName {
			err.SetError(fmt.Errorf("availabilitySetName: want %s, got %s", availabilitySetName, fnAvailabilitySetName))
			return
		}
		r, e := resp.Next()
		if e != nil {
...
func (a *AvailabilitySetsServerTransport) dispatchCreateOrUpdate(req *http.Request) (*http.Response, error) {
	if len(a.srv.onCreateOrUpdate) == 0 {
	...
	args := availabilitySetsCreateOrUpdateParams{
		// TODO: ctx
		ResourceGroupName:   matches[regex.SubexpIndex("resourceGroupName")],
		AvailabilitySetName: matches[regex.SubexpIndex("availabilitySetName")],
		Parameters:          body,
	}
	for _, reqr := range a.srv.onCreateOrUpdate {
		if resp := reqr.Match(args); resp != nil {
			// found a match, return its response
			res, err := resp.Next()
			if err != nil {
				return nil, err
			}
			return marshalResponseAsJSON(req, res)
		}
	}
	return nil, &nonRetriableError{fmt.Errorf("CreateOrUpdate: didn't find response for args %s", args)}
}
