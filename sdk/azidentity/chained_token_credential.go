...
func NewChainedTokenCredential(sources ...azcore.TokenCredential) (*ChainedTokenCredential, error) {
	if len(sources) == 0 {
		credErr := newCredentialUnavailableError("Chained Token Credential", "Length of sources cannot be 0")
	...
	for _, source := range sources {
		if source == nil { // cannot have a nil credential in the chain or else the application will panic when GetToken() is called on nil
			credErr := newCredentialUnavailableError("Chained Token Credential", "Sources cannot contain a nil TokenCredential")
			logCredentialError(credErr.CredentialUnavailable(), credErr)
			return nil, credErr
		}
	}
	...
	for _, cred := range c.sources {
		...
		if errors.As(err, &credErr) {
			...
			if errors.As(err, &authenticationFailed) {
				...
				authErr := newAuthenticationFailedError(fmt.Errorf("Received an AuthenticationFailedError, there is an invalid credential in the chain.\n%s\n%s"+createChainedErrorMessage(errList), err))
				return nil, authErr
			}
